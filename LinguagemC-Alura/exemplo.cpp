...
void chuta(){
    ...
    if(letra_existe(chute)){
        ...
        cout << "Voce acertou! Seu chute não está palavra." << endl;
...
vector<string> le_arquivo(){
    ...
    if(arquivo.is_open()){
        ...
        for(int i = 0; i < quantidade_palavras; i++){
            string palavras_lida;
            arquivo >> palavras_lida;
            palavras_do_arquivo.push_back(palavras_lida);
        }
        arquivo.close();
...
void salva_arquivo(const vector<string>& nova_lista){
    ofstream arquivo;
    arquivo.open("palavras.txt");
    if(arquivo.is_open()){
        ...
        cout << "Não foi posivel acessar o banco de palavras." << endl;
        exit(0);
    }
}
